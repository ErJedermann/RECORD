...
def read_output_csv(filename: str, old_output_setting: bool = False):
    filename = filename
    if os.path.exists(filename):
        ...
        with open(filename, 'r') as csvinput:
            ...
            for row in reader:
                ...
                all_rows.append(row)
        csvinput.close()
        if old_output_setting:
            print(f"WARNING: not switching column->row, due to old-output-setting activated!")
            all_colums = np.array(all_rows, dtype=float)
        else:
...
def combine_rec_outputs_to_plot(x_axis: [float], x_label: str, figure_name: str, output_folder: str,
                                output_mapping: [(int, int, str)], atk_types: [str], real_world_data: dict = None):
    ...
    for entry in output_mapping:
        ...
        some_data = read_output_csv(output_folder + temp_file, old_output_setting=False)
    ...
    plot_measurement_row.plot_attack_area_analysis_selfmade_quantiles(print_dict, x_axis, x_label=x_label, figure_name=figure_name, real_world_data=real_world_data)

if __name__ == '__main__':
    # real_world_times = [288, 683, 1028, 1463, 1715, 2085, 2318, 4044, 4237, 6190]
    real_world_times = ['3 min', '10 min', '10 min', '10 min', '30 min', '30 min', '30 min', '1h', '1h', '2h' ]
    real_world_areas = [102534, 81437, 81337, 17118, 88292, 3240, 47704, 8654, 43152, 383]
    real_world_data = {'times': real_world_times, 'estimations': real_world_areas}

    fig_name = f"Comparison of different attacker types vs. real-world measurements"
    ...
    combine_rec_outputs_to_plot(x_axis=x_values, x_label=x_text, figure_name=fig_name, output_folder=folder_name,
                                output_mapping=mapping, atk_types=attack_types, real_world_data=real_world_data)
